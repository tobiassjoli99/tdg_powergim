...
def get_wind_values(wind_rows, country_code):
    ...
    df = pd.read_csv('/Users/tobiassjoli/Documents/GitHub/powergim/only_changes_here/make_input_data/timeseries_and_consumers/NT_time_series/wind_series_raw.csv')
...
def get_hydro_values(hydro_rows):
    # Read the CSV file into a pandas dataframe
    df = pd.read_csv(
        '/Users/tobiassjoli/Documents/GitHub/powergim/only_changes_here/make_input_data/timeseries_and_consumers/DE_time_series/hydro_series_raw.csv',
        sep=';')
...
def make_NT_demand_time_series():
    ...
    for sheet_name in sheet_names:
        file_path = os.path.join(folder_path, sheet_name + '_demand_data_2030_NT.csv')
    ...
    df.to_csv('time_series_sample_NT.csv', index=False)
    ...
    df_con.to_csv('consumers_NT_new.csv', index=False)

    return df


if __name__ == "__main__":
    make_NT_demand_time_series()
